...
2.  **Payment Processing Race Conditions (TOCTOU Vulnerability)**
    *   **Finding:** Simultaneous calls to the webhook and the frontend success redirect would process the transaction twice, decrementing stock twice and rewriting invoice history. Standard `findUnique` statements failed to acquire true locks.
    *   **Fix:** Implemented strict row-level database locks. Before `PAID` transitions occur, the database query executes *inside* a `prisma.$transaction`, via raw `SELECT ... FOR UPDATE` row-locks against PostgreSQL to guarantee atomic operations and verify `paymentStatus !== 'PAID'` strictly.
3.  **Overselling / Stock Race Conditions**
    *   **Finding:** Checking if stock was available on order creation suffered the same concurrency gap. Multiple overlapping checkout sessions could drain stock below zero.
    *   **Fix:** Added `SELECT * FROM "Product" WHERE "id" = ... FOR UPDATE` directly in the creation transaction to row-lock product queries, verifying absolute quantities safely before Razorpay generation.
4.  **Financial Integrity (GST Rounding Mismatches)**
...
6.  **Security Configurations & Fallback Secrets**
...
## Final Statement
The platform has achieved a stable, defensible structure. It correctly processes edge cases such as closed tabs, duplicated callbacks, true PostgreSQL concurrency blocks, and floating-point fractional taxation. It is ready for peak-traffic customer routing.
